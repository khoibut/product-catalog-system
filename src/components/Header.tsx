...
function Header({ username, inverted }: HeaderProps) {
    ...
    return (
        <header className={`font-inter bg-black text-white py-4 text-lg px-30 flex fixed top-0 left-0 w-full border-b-1 border-b-white justify-between items-center ${inverted ? 'invert' : ''} z-10`}>
            <div className="flex gap-10">
                <div onClick={() => {
                    navigate.push("/products");
                }} className="font-bold text-xl hover:cursor-pointer">Ecommerce</div>
                <div className="hover:cursor-pointer">Shop</div>
                <div className="hover:cursor-pointer">Stories</div>
